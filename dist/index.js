...
function typeToPhp(type, optional = false) {
    ...
    if (n === "bytes")
        return optional ? "?string" : "string"; // PHP strings are binary-safe
    if (type.kind === "Enum")
        return optional ? "?string" : "string";
    if (isArrayType(type))
        return optional ? "?array" : "array";
...
function emitModelFunctions(m, L) {
    ...
    if (optional.length === 0) {
        ...
        for (const f of optional)
            L.push(`    if ($obj->${fieldPhp(f.name)} !== null) $count++;`);
    ...
    for (const f of fields) {
        ...
        if (f.optional) {
            ...
            for (const line of writeLines(f.type, `$obj->${fPhp}`, "        "))
                L.push(line);
            L.push(`    }`);
        }
        else {
...
export async function $onEmit(context) {
    ...
    if (checkAndReportReservedKeywords(program, services, ignoreReservedKeywords))
        return;
    const phpModelNs = new Map();
    for (const s of services) {
        for (const m of s.models) {
            if (m.name)
                phpModelNs.set(m.name, s.serviceName);
        }
        for (const e of s.enums) {
            if (e.name)
                phpModelNs.set(e.name, s.serviceName);
        }
        for (const u of s.unions) {
            if (u.name)
                phpModelNs.set(u.name, s.serviceName);
        }
    }
            ...
            for (const f of extractFields(m)) {
                const collectX = (t) => {
                    if ((t.kind === "Model" || t.kind === "Enum") && t.name) {
                        const ns = phpModelNs.get(t.name);
